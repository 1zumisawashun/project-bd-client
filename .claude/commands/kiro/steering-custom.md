...
2. **Check if template exists**:
   - Load from `docs/specs/settings/templates/steering-custom/{name}.md` if available
...
4. **Generate custom steering**:
   ...
   - Apply principles from `docs/specs/settings/rules/steering-principles.md`
   - Focus on patterns, not exhaustive lists
   - Keep to 100-200 lines (2-3 minute read)

5. **Create file** in `docs/specs/steering/{name}.md`

## Available Templates

Templates available in `docs/specs/settings/templates/steering-custom/`:
...
## Created:
- docs/specs/steering/api-standards.md
