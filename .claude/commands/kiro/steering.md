---
description: Manage docs/specs/steering/ as persistent project knowledge
...
**Role**: Maintain `docs/specs/steering/` as persistent project memory.
...
- All `docs/specs/steering/*.md` treated equally (core + custom)
  </background_information>

<instructions>
## Scenario Detection

Check `docs/specs/steering/` status:
...
1. Load templates from `docs/specs/settings/templates/steering/`
...
5. Load principles from `docs/specs/settings/rules/steering-principles.md`
...
1. Load all existing steering (`docs/specs/steering/*.md`)
...
From `docs/specs/settings/rules/steering-principles.md`:
...
- All `docs/specs/steering/*.md` loaded as project memory
- Templates and principles are external for customization
- Focus on patterns, not catalogs
- "Golden Rule": New code following patterns shouldn't require steering updates
- Avoid documenting agent-specific tooling directories (e.g. `.cursor/`, `.gemini/`, `.claude/`)
- `docs/specs/settings/` content should NOT be documented in steering files (settings are metadata, not project knowledge)
- Light references to `docs/specs/specs/` and `docs/specs/steering/` are acceptable; avoid other `.kiro/` directories
