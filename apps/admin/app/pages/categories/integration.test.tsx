...
describe("Admin Category Management Integration", () => {
  ...
  describe("カテゴリー作成フロー", () => {
    it("管理者としてカテゴリーを作成できる", async () => {
      ...
      // フォームに入力
      await user.type(screen.getByLabelText("カテゴリー名", { exact: false }), "Technology");
    ...
    it("重複カテゴリー名でエラーが表示される", async () => {
      ...
      const user = userEvent.setup();
      render(
        <AdminCategoriesPage categories={[{ id: "1", name: "Existing", createdAt: new Date() }]} />,
      );

      // 既存のカテゴリー名を入力
      await user.type(screen.getByLabelText("カテゴリー名", { exact: false }), "Existing");
      await user.click(screen.getByRole("button", { name: "作成" }));

      // エラーメッセージが表示される
      await waitFor(() => {
        expect(screen.getByText("このカテゴリー名は既に存在します")).toBeInTheDocument();
      });
    });
  });
  ...
  describe("カテゴリー一覧表示", () => {
    ...
    it("カテゴリーがない場合は空状態メッセージが表示される", () => {
      ...
      expect(screen.getByText("カテゴリーがまだ作成されていません")).toBeInTheDocument();
    });
  });
  ...
  describe("ローディング状態", () => {
    it("送信中はボタンが無効化される", async () => {
      ...
      await user.type(screen.getByLabelText("カテゴリー名", { exact: false }), "Test");
      await user.click(screen.getByRole("button", { name: "作成" }));
