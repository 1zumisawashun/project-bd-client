...
const checkRefetchQuery = (context: Context, node: TSESTree.ObjectExpression) => {
...
const checkRefetchQueries = (context: Context, node: TSESTree.ArrayExpression) => {
        ...
        if (isHook(node)) {
          ...
          if (isIdentifier(property.key)) {
            ...
            refetchQueries?.references.forEach((r) => {
              const parent = r.identifier.parent;
              if (isVariableDeclarator(parent) && isArrayExpression(parent.init)) {
